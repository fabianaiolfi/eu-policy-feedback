...
#### Seed Word Selection: Own Seed Words

In a first approach, I create my own seed words:
...
#### Seed Word Selection: Systematic Approach
...
The following tables display the top and bottom of the tables displaysing extracted Wordscore scores for terms extracted from the manifestos. Negative values are associated with left-wing characteristics, positive values with right-wing subjects.
...
wordscores_manifesto_ngram_1 <- wordscores_manifesto_ngram_1 %>% arrange(wordscores_manifesto)
wordscores_manifesto_ngram_2 <- wordscores_manifesto_ngram_2 %>% arrange(wordscores_manifesto)
wordscores_manifesto_ngram_3 <- wordscores_manifesto_ngram_3 %>% arrange(wordscores_manifesto)
wordscores_manifesto_ngram_4 <- wordscores_manifesto_ngram_4 %>% arrange(wordscores_manifesto)
```

**Unigrams**
```{r echo=FALSE, message=FALSE, warning=FALSE}
head(wordscores_manifesto_ngram_1)
tail(wordscores_manifesto_ngram_1)
```

**Bigrams**
```{r echo=FALSE, message=FALSE, warning=FALSE}
head(wordscores_manifesto_ngram_2)
tail(wordscores_manifesto_ngram_2)
```

**Trigrams**
```{r echo=FALSE, message=FALSE, warning=FALSE}
head(wordscores_manifesto_ngram_3)
tail(wordscores_manifesto_ngram_3)
```

**4-grams**
```{r echo=FALSE, message=FALSE, warning=FALSE}
head(wordscores_manifesto_ngram_4)
tail(wordscores_manifesto_ngram_4)
```
...
I then examine the most frequent keywords in each bin.
...
*Note: Using the full dataset (ca. 70k observations) showed almost identical results. Also using a larger GloVe model, e.g. 300 dimensions, doesn’t alter results in a way that justifies the longer computational time.*
