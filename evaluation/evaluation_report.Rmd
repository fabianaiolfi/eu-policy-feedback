...
The scatter plots show that the variance of the calculated scores is low. I examine this in more detail in the following plots. They show that the variance of the computed scores is generally lower than the variance of the ground truth. I address this issue further below.
...
### Discussion
Looking at the adjusted MAE with variance penalties, we can see that the Bakker Hobolt Social method used by @simonhixPoliticalIdeologyPolicyLoss2024 is closest to the ground truth, both when using a policy’s summary and its preamble. The Bakker Hobolt Social method uses a fine-tuned model, trained on data from the [Manifesto Project](https://manifesto-project.wzb.eu/). It is therefore well equipped to perform well in the specific task of scoring EU policies.

In second and third place is the LSS method. This is an interesting finding, as it suggests that these relatively old natural language processing methods are closer to the ground truth than state-of-the-art large language models.

Possible reasons why the LLMs perform worse than the Bakker Hobolt Social or LSS methods are that they are not fine-tuned and are not designed to perform particularly well in classifying EU policies. I also assume that non-fine-tuned LLMs produce a lot of noise, leading to worse results than the LSS method, which is "fine-tuned" in its own way with its seed words.
