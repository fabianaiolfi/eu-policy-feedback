...
#### Ranking
The ranking method compares two policies. Two random policies are presented to the LLM, which decides which policy is more economically left (or right) based on set criteria. The “winning” policy is flagged. Repeating this process many times and recording “winning” and “losing” policies enables the use of the [Elo rating system](https://en.wikipedia.org/wiki/Elo_rating_system) to calculate a score for each policy. Economically left “winning” policies receive a higher “left” score than economically left “losing” policies.

Ranking was only performed on the summaries. Applying this method to the preambles, i.e., over 70k documents, would have taken several dozen hours of inference and over $100 in API costs when using ChatGPT.
...
The ground truth exhibits much higher variance than the calculated scores (see histograms and boxplots above under “Variance”). To address this, I introduce a variance penalty that penalizes scores for their lack of variance. The MAE is adjusted by multiplying it by the inverse of the variance ratio. The variance ratio is the ratio of the ground truth variance to the calculated scores variance. Scores with higher variance are less penalized, while scores with lower variance are more penalized.
