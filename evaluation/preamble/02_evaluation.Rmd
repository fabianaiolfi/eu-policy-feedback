...
```{r, include=FALSE}
broad_policy_mpolicy_avg_df <- readRDS(here("data", "evaluation", "broad_policy_mpolicy_avg_df_preamble_raw_results.rds"))
broad_policy_mpolicy_avg_df <- broad_policy_mpolicy_avg_df %>% 
  ...
  mutate_at(c('avg_lss_econ_z_score',
              ...
              'llama_preamble_0_shot_z_score',
              'chatgpt_preamble_0_shot_z_score'
              ), as.numeric) %>% 
  ...
  rename("Llama 0-Shot" = llama_preamble_0_shot_z_score) %>% 
  rename("ChatGPT 0-Shot" = chatgpt_preamble_0_shot_z_score)
```
...
boxplot_data <- broad_policy_mpolicy_avg_df %>% 
  ...
  dplyr::filter(
      ...
      z_score_measurement == "ChatGPT 0-Shot"
      )
