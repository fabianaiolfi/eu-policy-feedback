...
*Red dashed line of equality shows where the values from the preamble and the summary would be equal. Points below the line indicate that preamble value is greater than the summary value (i.e., more right), and points above indicate the opposite.*  
*Plot applies jitter in order to counteract overlapping points (e.g., at `0,0`).*
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(comparison, aes(x = RoBERT_left_right_preamble, y = RoBERT_left_right_summary)) +
  geom_jitter(size = 1.5, alpha = 0.5, width = 0.1, height = 0.1) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
  labs(x = "Preamble", y = "Summary", title = "") +
  xlim(-6, 3) +
  ylim(-6, 3) +
  theme_minimal()
```
...
*Red dashed line of equality shows where the values from the preamble and the summary would be equal. Points below the line indicate that preamble value is lower than the summary value (i.e., more left), and points above indicate the opposite.*  
*Plot applies jitter in order to counteract overlapping points.*
```{r echo=FALSE, message=FALSE, warning=FALSE}
df <- comparison %>%
  mutate(mean_value = (RoBERT_left_right_preamble + RoBERT_left_right_summary) / 2,
         difference = RoBERT_left_right_preamble - RoBERT_left_right_summary)

ggplot(df, aes(x = mean_value, y = difference)) +
  geom_jitter(size = 1.5, alpha = 0.5, width = 0.1, height = 0.1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(x = "Mean of Preamble and Summary", y = "Difference (Preamble - Summary)", title = "") +
  theme_minimal()
